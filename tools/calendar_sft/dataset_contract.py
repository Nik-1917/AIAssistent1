...
def _validate_add(
    ...
    if "ends_at" in params:
        end = datetime.fromisoformat(_parse_datetime(params["ends_at"], f"{location}.params.ends_at"))
        if not has_starts_at and not (has_date and has_time):
            _fail(f"{location}.params.ends_at", "requires a complete start")
        start_text = params["starts_at"] if has_starts_at else params["date"] + "T" + params["time"]
        start = datetime.fromisoformat(_parse_datetime(start_text, f"{location}.params.starts_at"))
        minutes = (end - start).total_seconds() / 60
        if not 0 < minutes <= 2147483647:
            _fail(f"{location}.params.ends_at", "must be later than start within the supported duration")
        if "duration_min" in params and params["duration_min"] != minutes:
            _fail(f"{location}.params.ends_at", "conflicts with duration_min")
    if "value" in params:
