...
UPDATE_SCENARIOS = (
    ("мастера маникюра", "Запись клиентки на маникюр", "запись клиентки на маникюр", "Маникюр клиентке у Елены"),
    ("парикмахера", "Стрижка клиента", "стрижка клиента", "Стрижка клиента в салоне"),
    ("врача", "Приём пациента", "приём пациента", "Консультация пациента"),
    ("чиновника", "Приём граждан", "приём граждан", "Приём жителей района"),
    ("рабочего", "Смена на стройке", "смена на стройке", "Смена на объекте"),
    ("крестьянина", "Выезд в поле", "выезд в поле", "Осмотр поля"),
    ("спортсмена", "Тренировка в бассейне", "тренировка в бассейне", "Тренировка по плаванию"),
    ("офисного сотрудника", "Созвон с отделом", "созвон с отделом", "Созвон по проекту"),
    ("таксиста", "Техосмотр такси", "техосмотр такси", "Техосмотр автомобиля"),
    ("уборщицы", "Уборка подъезда", "уборка подъезда", "Уборка первого подъезда"),
    ("водителя", "Рейс в Тольятти", "рейс в Тольятти", "Рейс в Тольятти утром"),
    ("преподавателя", "Урок алгебры", "урок алгебры", "Урок алгебры для десятого класса"),
    ("учащегося", "Консультация по курсовой", "консультация по курсовой", "Консультация по курсовой работе"),
    ("соцработника", "Визит к подопечной", "визит к подопечной", "Визит к подопечной Анне"),
...
def make_previous_month_phrase_searches(anchor: datetime, titles: tuple[str, ...], total: int, start_index: int = 0) -> list[dict]:
    ...
    for index in range(start_index, start_index + total):
        ...
        params = {"query": title.lower(), "range_start": f"{start:%Y-%m-%d}T00:00", "range_end": f"{end:%Y-%m-%d}T00:00"}
        if template_index != 1:
            params["query"] = ""
